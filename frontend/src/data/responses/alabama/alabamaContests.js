export default contests = [
	{
		office: 'President of the United States',
		official: {
			name: 'Joseph R. Biden',
			party: 'Democratic Party',
			phones: ['(202) 456-1111'],
			urls: [
				'https://www.whitehouse.gov/',
				'https://en.wikipedia.org/wiki/Joe_Biden',
			],
		},
	},
	{
		office: 'Vice President of the United States',
		official: {
			name: 'Kamala D. Harris',
			party: 'Democratic Party',
			phones: ['(202) 456-1111'],
			urls: [
				'https://www.whitehouse.gov/',
				'https://en.wikipedia.org/wiki/Kamala_Harris',
			],
		},
	},
	{
		office: 'U.S. Senator',
		official: {
			name: 'Richard C. Shelby',
			party: 'Republican Party',
			phones: ['(202) 224-5744'],
			urls: [
				'https://www.shelby.senate.gov/',
				'https://en.wikipedia.org/wiki/Richard_Shelby',
			],
			photoUrl:
				'http://bioguide.congress.gov/bioguide/photo/S/S000320.jpg',
		},
	},
	{
		office: 'U.S. Senator',
		official: {
			name: 'Tommy Tuberville',
			party: 'Republican Party',
			phones: ['(202) 224-4124'],
			urls: [
				'https://tuberville.senate.gov/',
				'https://en.wikipedia.org/wiki/Tommy_Tuberville',
			],
		},
	},
	{
		office: 'U.S. Representative',
		official: {
			name: 'Mike D. Rogers',
			party: 'Republican Party',
			phones: ['(202) 225-3261'],
			urls: [
				'https://mikerogers.house.gov/',
				'https://en.wikipedia.org/wiki/Mike_D._Rogers',
			],
			photoUrl:
				'http://bioguide.congress.gov/bioguide/photo/R/R000575.jpg',
		},
	},
	{
		office: 'Governor of Alabama',
		official: {
			name: 'Kay Ivey',
			party: 'Republican Party',
			phones: ['(334) 242-7100'],
			urls: [
				'https://governor.alabama.gov/',
				'https://en.wikipedia.org/wiki/Kay_Ivey',
			],
			photoUrl:
				'http://governor.alabama.gov/uploads/images/_size/Portrait.jpeg',
			emails: ['constituent.services@governor.alabama.gov'],
		},
	},
	{
		office: 'Lieutenant Governor of Alabama',
		official: {
			name: 'Will Ainsworth',
			party: 'Republican Party',
			phones: ['(334) 261-9590'],
			urls: [
				'https://ltgov.alabama.gov/',
				'https://en.wikipedia.org/wiki/Will_Ainsworth',
			],
			emails: ['ltgov@ltgov.alabama.gov'],
		},
	},
	{
		office: 'AL State Auditor',
		official: {
			name: 'Jim Zeigler',
			party: 'Republican Party',
			phones: ['(334) 242-7010'],
			urls: [
				'http://auditor.alabama.gov/',
				'https://en.wikipedia.org/wiki/Jim_Zeigler',
			],
			emails: ['jim.zeigler@auditor.alabama.gov'],
		},
	},
	{
		office: 'AL State Attorney General',
		official: {
			name: 'Steve Marshall',
			party: 'Republican Party',
			phones: ['(334) 242-7300'],
			urls: [
				'https://www.alabamaag.gov/',
				'https://en.wikipedia.org/wiki/Steve_Marshall_%28politician%29',
			],
			emails: ['constituentaffairs@ago.state.al.us'],
		},
	},
	{
		office: 'AL Secretary of State',
		official: {
			name: 'John H. Merrill',
			party: 'Republican Party',
			phones: ['(334) 242-7200'],
			urls: [
				'http://sos.alabama.gov/',
				'https://en.wikipedia.org/wiki/John_Merrill_%28American_politician%29',
			],
			emails: ['john.merrill@sos.alabama.gov'],
		},
	},
	{
		office: 'AL State Commissioner of Agriculture and Industries',
		official: {
			name: 'Rick Pate',
			party: 'Republican Party',
			phones: ['(334) 240-7100'],
			urls: [
				'http://agi.alabama.gov/',
				'https://en.wikipedia.org/wiki/Rick_Pate',
			],
			emails: ['rick.pate@agi.alabama.gov'],
		},
	},
	{
		office: 'AL State Treasurer',
		official: {
			name: 'Young Boozer',
			party: 'Republican Party',
			phones: ['(334) 242-7500'],
			urls: [
				'https://treasury.alabama.gov/',
				'https://en.wikipedia.org/wiki/Young_Boozer',
			],
			emails: ['alatreas@treasury.alabama.gov'],
		},
	},
	{
		office: 'AL State Public Service Commission President',
		official: {
			name: 'Twinkle Andress Cavanaugh',
			party: 'Republican Party',
			phones: ['(334) 242-5297'],
			urls: [
				'http://www.psc.alabama.gov/Cavanaugh/commissionercavanaugh.htm',
			],
			emails: ['twinkle.cavanaugh@psc.alabama.gov'],
		},
	},
	{
		office: 'AL State Public Service Commissioner',
		official: {
			name: 'Chip Beeker',
			party: 'Republican Party',
			phones: ['(334) 242-5191'],
			urls: ['http://www.psc.alabama.gov/Beeker/commissionerBeeker.htm'],
			emails: ['chip.beeker@psc.alabama.gov'],
		},
	},
	{
		office: 'AL State Public Service Commissioner',
		official: {
			name: 'Jeremy H. Oden',
			party: 'Republican Party',
			phones: ['(334) 242-5203'],
			urls: [
				'http://www.psc.alabama.gov/Oden/CommissionerOden.htm',
				'https://en.wikipedia.org/wiki/Jeremy_Oden',
			],
			emails: ['jeremy.oden@psc.alabama.gov'],
		},
	},
	{
		office: 'AL State Senator',
		official: {
			name: 'Tom Whatley',
			party: 'Republican Party',
			phones: ['(334) 261-0893'],
			urls: [
				'https://alison.legislature.state.al.us/senate-leaders-members?tab=1',
				'https://en.wikipedia.org/wiki/Tom_Whatley',
			],
			photoUrl:
				'http://www.legislature.state.al.us/ALISWWW/Senate/Senator%20Pictures/Whatley_27.jpeg',
			emails: ['tom.whatley@alsenate.gov'],
		},
	},
	{
		office: 'AL State Representative',
		official: {
			name: 'Joe Lovvorn',
			party: 'Republican Party',
			phones: ['(334) 261-0540'],
			urls: [
				'https://alison.legislature.state.al.us/house-leaders-members?tab=1',
			],
			photoUrl:
				'http://www.legislature.state.al.us/ALISWWW/House/Representative%20Pictures/Lovvorn_79.jpeg',
			emails: ['joe.lovvorn@alhouse.gov'],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Alisa Kelli Wise',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Alisa_Kelli_Wise',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Brady E. Mendheim, Jr.',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Brady_E._Mendheim_Jr.',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Greg Shaw',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Greg_Shaw_%28judge%29',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Jay Mitchell',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Jay_Mitchell_%28judge%29',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Michael F. Bolin',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Michael_F._Bolin',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Sarah H. Stewart',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Sarah_Hicks_Stewart',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'Tommy Bryan',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Tommy_Bryan',
			],
		},
	},
	{
		office: 'AL State Supreme Court Associate Justice',
		official: {
			name: 'William B. Sellers',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Will_Sellers',
			],
		},
	},
	{
		office: 'AL State Supreme Court Chief Justice',
		official: {
			name: 'Tom Parker',
			party: 'Republican Party',
			phones: ['(334) 229-0700'],
			urls: [
				'http://judicial.alabama.gov/Appellate/SupremeCourt',
				'https://en.wikipedia.org/wiki/Tom_Parker_%28judge%29',
			],
		},
	},
	{
		office: 'Lee County Probate Judge',
		official: {
			name: 'Bill English',
			party: 'Republican Party',
			phones: ['(334) 737-3670'],
			urls: ['http://www.leeco.us/government/probate_judge/index.php'],
			emails: ['probatejudge@mindspring.com'],
		},
	},
	{
		office: 'Lee County Circuit Clerk',
		official: {
			name: 'Mary B. Roberson',
			party: 'Republican Party',
			phones: ['(334) 749-7141'],
			urls: ['https://lee.alacourt.gov/'],
		},
	},
	{
		office: 'Lee County Sheriff',
		official: {
			name: 'Jay Jones',
			party: 'Republican Party',
			phones: ['(334) 749-5651'],
			urls: ['http://leecountysheriff.org/'],
			emails: ['lcso@leecountysheriff.org'],
		},
	},
	{
		office: 'Lee County Coroner',
		official: {
			name: 'Daniel Sexton',
			party: 'Unknown',
			phones: ['(334) 737-3620'],
			urls: ['https://www.leecountycoroner.org/'],
			emails: ['help@leecountycoroner.org'],
		},
	},
	{
		office: 'Lee County Commissioner',
		official: {
			name: 'Ross Morris',
			party: 'Unknown',
			phones: ['(334) 737-3660'],
			urls: [
				'http://www.leeco.us/government/county_commission/index.php',
			],
		},
	},
	{
		office: 'Lee County Revenue Commissioner',
		official: {
			name: 'Oline Price',
			party: 'Republican Party',
			phones: ['(334) 737-3655'],
			urls: ['https://www.leecountyrevenuecommissioner.com/'],
			emails: ['oprice@leeco.us'],
		},
	},
];
